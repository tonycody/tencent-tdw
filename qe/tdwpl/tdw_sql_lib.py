...
def trim(tdw, old_str, ch = None):
    ...
    old_str = ltrim(tdw, old_str, ch)
    return rtrim(tdw, old_str, ch)
...
def to_char(tdw, uinput, uformat = None):
    ...
    if isinstance(uinput, Dateclass):
        return to_char_date(tdw, uinput, uformat)
    else:
        try:
...
def last_day(tdw, dc):
    ...
    if not isinstance(dc, Dateclass):
        ...
        return None
    tc = calendar.monthrange(int(dc.year), int(dc.month))
...
def next_day(tdw, dateclass, wd):
    ...
    l = list()
    c = calendar.monthcalendar(int(dateclass.year), int(dateclass.month))
