...
select unionsrc.key, count(1) FROM (select 'tst1' as key, cast(count(1) as string) as value from src s1                                       UNION  ALL                                    select s2.key , s2.value from src1 s2) unionsrc group by unionsrc.key;
