...
public class RCFile {
  ...
  public static final String TOLERATE_CORRUPTIONS_CONF_STR = "hive.io.rcfile.tolerate.corruptions";
  ...
  private static final byte[] ORIGINAL_MAGIC = new byte[] { (byte) 'S',
      (byte) 'E', (byte) 'Q' };
  ...
  private static final byte[] ORIGINAL_MAGIC_VERSION = new byte[] { (byte) 'S',
      (byte) 'E', (byte) 'Q', ORIGINAL_MAGIC_VERSION_WITH_METADATA };

  // The 'magic' bytes at the beginning of the RCFile
  private static final byte[] MAGIC = new byte[] { (byte) 'R', (byte) 'C',
      (byte) 'F' };
    ...
    class LazyDecompressionCallbackImpl implements LazyDecompressionCallback {
      ...
      public byte[] decompress() throws IOException {
        ...
        if (deflatFilter instanceof SchemaAwareCompressionInputStream) {
          ((SchemaAwareCompressionInputStream) deflatFilter)
  ...
  public static class Writer {
     ...
     * @param replication
     *          the number of replicas for the file
     * @param blockSize
     *          the block size of the file
     * @param progress
     *          the progress meter for writing the file
     * @param metadata
     *          a string to string map in the file header
     * @throws IOException
     */
    public Writer(FileSystem fs, Configuration conf, Path name, int bufferSize,
      ...
      conf.setInt("io.compression.codec.lzo.buffersize", 128 * 1024);
    ...
    void init(Configuration conf, FSDataOutputStream out,
      ...
      this.useNewMagic = conf.getBoolean(
          HiveConf.ConfVars.HIVEUSEEXPLICITRCFILEHEADER.varname, true);
    }
     ...
     * {@link BytesRefArrayWritable}. If its <code>size()</code> is less than
     * the column number in the file, zero bytes are appended for the empty
     * columns. If its size() is greater then the column number in the file, the
     * exceeded columns' bytes are ignored.
     * 
     * @param val
    ...
    private void flushRecords() throws IOException {
      rowCount = rowCount + bufferedRecords;
    ...
    private void writeKey(KeyBuffer keyBuffer, int recordLen, int keyLength)
        throws IOException {
      ...
      if (this.isCompressed()) {
        ...
        CompressionOutputStream deflateFilter = codec.createOutputStream(
            compressionBuffer, compressor);
    ...
    public synchronized void close() throws IOException {
      ...
      for (int i = 0; i < columnNumber; i++) {
        LOG.info("Column#" + i + " : Plain Total Column Value Length: "
            ...
            + ",  Compr Total Column Value Length: "
            + comprTotalColumnLength[i]);
      }
  ...
  public static class Reader {
    ...
    public Reader(FileSystem fs, Path file, int bufferSize, Configuration conf,
      ...
      tolerateCorruptions = conf.getBoolean(TOLERATE_CORRUPTIONS_CONF_STR,
          false);
     ...
     * 
     * This can be used after one calls seek or sync, if one called next before
     * that. Otherwise, the seek or sync will have no effect, it will continue
     * to get rows from the buffer built up from the call to next.
     */
      ...
      try {
        ...
        while (n > 0 && (in.getPos() + n) <= end) {
          ...
          for (int i = 0; i < n; i++) {
            ...
            if (j == sync.length) {
              /*
               * simplified from (position + (i - prefix) + sync.length) -
               * SYNC_SIZE
               */
     ...
     * 
     * @param columnID
     *          the number of the column to get 0 to N-1
     * @throws IOException
     */
    public BytesRefArrayWritable getColumn(int columnID,
      ...
      fetchColumnTempBuf.reset(
          currentKey.allCellValLenBuffer[columnID].getData(),
          currentKey.allCellValLenBuffer[columnID].getLength());
    ...
    private int nextKeyValueTolerateCorruptions() throws IOException {
      ...
      try {
        ...
        LOG.warn("Ignoring IOException in file " + file + " after offset "
            + currentOffset, ioe);
        ret = -1;
      } catch (Throwable t) {
        ...
        LOG.warn("Ignoring unknown error in " + file + " after offset "
            + currentOffset, t);
        ret = -1;
      }
      return ret;
     ...
     * 
     * @param selCol
     *          - index among selectedColumns
     * @param col
     *          - column object to update the state of. prvLength will be set to
     *          the new read position
     * @throws IOException
     */
    private void colAdvanceRow(int selCol, SelectedColumn col)
        throws IOException {
      if (col.runLength > 0) {
        --col.runLength;
      } else {
        int length = (int) WritableUtils
            .readVLong(colValLenBufferReadIn[selCol]);
        if (length < 0) {
    ...
    public void close() {
      ...
      LOG.info("RCFile Reader Row Count::" + this.rowCount);
    }
